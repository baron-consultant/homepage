...
$(function () {
  ...
  $(".btn_more").on("click", function () {
    ...
    const isAlreadyInserted = $next.hasClass("popup_wrap") && $next.hasClass(value);
                ...
                <div class="popup_contents_wrap" data-lenis-prevent-wheel>
                  <picture>
                    <source
                      ...
                      srcset="../assets/img/dt/m_${pageType}_${value}.jpg"
                    />
                    <source
                      media="(min-width: 1401px)"
                      srcset="../assets/img/dt/${pageType}_${value}.jpg"
                    /> 
                  <img src="../assets/img/dt/${pageType}_${value}.jpg">
                </picture>
                </div>
    ...
    const observer = new IntersectionObserver(
      (entries, obs) => {
        if (entries.some((entry) => entry.isIntersecting)) {
          requestAnimationFrame(initSvgLine);
          obs.disconnect();
        }
      },
      { rootMargin: "200px 0px" }
    );
    observer.observe(bgLine);
  } else {
    requestAnimationFrame(initSvgLine);
  }
});
