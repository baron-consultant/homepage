...
// ── 헤더 + floating_menu 애니메이션 (GSAP/ScrollTrigger가 있을 때만)
if (window.gsap) {
  try {
    ...
    // 헤더 애니메이션
    const showNav = document.querySelector(".js__header")
      ? gsap
          .from(".js__header", {
            yPercent: -200,
            paused: true,
    ...
    // floating_menu 애니메이션 (오른쪽으로 사라짐)
    const hideFloatingMenu = document.querySelector(".floating_menu")
      ? gsap
          .from(".floating_menu", {
            xPercent: 200,
            paused: true,
    ...
    const createScrollTrigger = () => {
      ...
      scrollTriggerInstance = ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          // 1200px 이하에서만 floating_menu 동작
          if (hideFloatingMenu && window.innerWidth <= 1200) {
            self.direction === -1
