window.onload = function() {
    $.ajax({
        ...
        complete: function() {
            ...
            // // 전송완료
            // $(document).ready(function() {
            //     $('.pw .join_btn_wrap button').click(function() {
            //         $('.contents_wrap').children().not('.messages').hide();
            //         $('.messages').show();
            //     });
            // });
            ...
            // 로그인시 overflow:hidden 오류 수정
            $(document).ready(function() {
                function toggleBodyOverflowLogin() {
                    const displayStatus = $('#pop_login').css('display');
                    console.log('pop_login display:', displayStatus); // 디버깅 출력
                ...
                // 초기 페이지 로드 시 한 번 실행
                setTimeout(toggleBodyOverflowLogin, 100); // 100ms 지연 후 실행

                // MutationObserver 설정
                const targetNode = document.getElementById('pop_login');
                const observer = new MutationObserver(function(mutationsList) {
                    mutationsList.forEach(function(mutation) {
                        if (mutation.attributeName === 'class' || mutation.attributeName === 'style') {
                            // #pop_login의 클래스나 스타일이 변경될 때마다 100ms 지연 후 함수 호출
                            setTimeout(toggleBodyOverflowLogin, 100);
                        }
                    });
                });

                // 감시할 속성 설정 (class와 style 속성 변경을 감시)
                observer.observe(targetNode, { attributes: true, attributeFilter: ['class', 'style'] });
            });

            // 회원가입시 overflow:hidden 오류 수정
            $(document).ready(function() {
                function toggleBodyOverflowJoin() {
                    const displayStatus = $('#pop_join').css('display');
                    console.log('pop_join display:', displayStatus); // 디버깅 출력
                ...
                // 초기 페이지 로드 시 한 번 실행
                setTimeout(toggleBodyOverflowJoin, 100); // 100ms 지연 후 실행

                // MutationObserver 설정
                const targetNode = document.getElementById('pop_join');
                const observer = new MutationObserver(function(mutationsList) {
                    mutationsList.forEach(function(mutation) {
                        if (mutation.attributeName === 'class' || mutation.attributeName === 'style') {
                            // #pop_join 클래스나 스타일이 변경될 때마다 100ms 지연 후 함수 호출
                            setTimeout(toggleBodyOverflowJoin, 100);
                        }
                    });
                });
